...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("Kampus") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "Kampus", DisplayName = "Kampus API", Resources = { "Kampus" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        var mauiClientId = configurationSection["Kampus_MauiNormal:ClientId"];
        if (!mauiClientId.IsNullOrWhiteSpace())
        {
            var mauiRootUrl = configurationSection["Kampus_MauiNormal:RootUrl"];
            await CreateApplicationAsync(
                ...
                secret: configurationSection["Kampus_MauiNormal:ClientSecret"] ?? "rastgelebisileruretilecek",
        ...
        var mauiBlazorClientId = configurationSection["Kampus_MauiBlazor:ClientId"];
        if (!mauiBlazorClientId.IsNullOrWhiteSpace())
        {
            //var mauiRootUrl = configurationSection["Kampus_MauiBlazor:RootUrl"];
            var mauiRootUrl = "kampuste://";
            await CreateApplicationAsync(
                    name: mauiBlazorClientId!,
                    type: OpenIddictConstants.ClientTypes.Confidential,
                    consentType: OpenIddictConstants.ConsentTypes.Implicit,
                    displayName: "Maui Blazor Application",
                    secret: configurationSection["Kampus_MauiBlazor:ClientSecret"] ?? "rastgelebisileruretilecek",
                    grantTypes: new List<string>
                    {
                        OpenIddictConstants.GrantTypes.AuthorizationCode,
                        OpenIddictConstants.GrantTypes.RefreshToken,
        ...
        var blazorServerTieredClientId = configurationSection["Kampus_BlazorServerTiered:ClientId"];
        if (!blazorServerTieredClientId.IsNullOrWhiteSpace())
        {
            var blazorServerTieredRootUrl = configurationSection["Kampus_BlazorServerTiered:RootUrl"]!.EnsureEndsWith('/');
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["Kampus_BlazorServerTiered:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["Kampus_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["Kampus_Swagger:RootUrl"]?.TrimEnd('/');
